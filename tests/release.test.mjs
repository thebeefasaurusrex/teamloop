...
async function check(entries) {
  const folder = await tempDir('teamloop-release-check-');
...
test('release check hashes allowlisted PNG and font files without decoding binary as text', async () => {
  ...
  const result = await check({
    'brand/logo.png': png,
    'brand/font.ttf': Buffer.from([0, 1, 0, 0, 255]),
    'brand/font.woff2': Buffer.from([119, 79, 70, 50, 255]),
  });
  assert.equal(result.status, 0, result.stderr);
  const report = JSON.parse(result.stdout);
  assert.equal(report.files, 5);
  assert.match(report.limits, /not text screening/);
});
test('release check rejects binary files that do not match their approved type', async () => {
  const result = await check({ 'brand/logo.png': Buffer.from('Not a PNG') });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Invalid binary header/);
});
test('release check rejects unsupported non-UTF8 files without crashing', async () => {
  const result = await check({ 'brand/file.bin': Buffer.from([255, 254, 253]) });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Non-UTF8 file without an approved binary type/);
});
test('release check still screens text for credentials beside binary assets', async () => {
  const result = await check({ 'brand/readme.md': ['ghp', 'x'.repeat(30)].join('_') });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Possible credential/);
});
test('release check rejects personal email addresses but allows documentation placeholders', async () => {
  // Built at runtime so this test file does not itself contain an address the release check would flag.
  const leaked = await check({ 'docs/notes.md': 'Maintainer: ' + ['someone', 'private-domain.io'].join('@') });
  assert.notEqual(leaked.status, 0);
  assert.match(leaked.stderr, /Personal email address in docs\/notes\.md: s\*\*\*@private-domain\.io/);
  const placeholder = await check({ 'docs/notes.md': 'Use operator@example.test and 1234+name@users.noreply.github.com in examples.' });
  assert.equal(placeholder.status, 0, placeholder.stderr);
});
test('release check ignores a git worktree pointer file', async () => {
  const result = await check({ '.git': Buffer.from('gitdir: ../private-worktree-metadata') });
  assert.equal(result.status, 0, result.stderr);
});
