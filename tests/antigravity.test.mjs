...
test('Antigravity requires privacy, billing, and deny rules', () => {
  const settings = {
    ...
    permissions: { deny: ['read_file(*)', 'write_file(*)', 'read_url(*)', 'execute_url(*)', 'command(*)', 'unsandboxed(*)', 'mcp(*)'] },
  };
  ...
  for (const change of [
    ...
    assert.throws(() => checkGeminiSettings({ ...settings, ...change }));
});
test('Antigravity sparse persistence is repaired without overriding unsafe explicit values', async () => {
  const home = await tempDir('team-loop-agy-settings-');
...
test('Antigravity selects file-backed personal login mode without exporting a token', () => {
  ...
  assert.deepEqual(geminiEnv({ PATH: 'test', USERPROFILE: 'C:/Users/example' }, config), {
    PATH: 'test',
    USERPROFILE: 'C:/Users/example',
    HOME: path.resolve('C:/Users/example'),
    SSH_CONNECTION: '127.0.0.1 49152 127.0.0.1 22',
  });
  assert.throws(
    () => resolveUserHome({ antigravityHome: 'C:/wrong/.gemini/antigravity-cli' }, { USERPROFILE: 'C:/Users/example' }),
    /personal OS account home/,
  );
});
test('reviewer installs at the documented global agent location and verifies exact content', async () => {
  const home = await tempDir('team-loop-agy-home-');
  ...
  assert.deepEqual(files, [
    path.join(home, '.gemini', 'config', 'agents', 'team-loop-reviewer', 'agent.md'),
    path.join(home, '.gemini', 'config', 'agents', 'team-loop-designer', 'agent.md'),
  ]);
...
test('Antigravity requires matching model, agent, and restrictive permission mode', () => {
  for (const change of [{ model: 'other' }, { agent: 'default' }, { permission_mode: 'always-proceed' }])
    assert.throws(() => guardGeminiEvent({ ...init, init: { ...init.init, ...change } }, model));
});
...
test('Antigravity structured finish protocol accepts streamed prefixes and terminal metadata', () => {
  ...
  const finishTool = {
    event: 'step_update',
    step_update: { step_index: 2, step_type: 'tool', tool_name: 'finish', tool_info: { name: 'finish', parameters: review }, state: 'DONE' },
  };
  ...
  const fullArtifact = {
    packetHash: 'abc',
    artifact: { filename: 'test.html', mediaType: 'text/html', content: '<!doctype html><html><body>complete</body></html>' },
    rationale: 'Complete.',
    uncertainties: [],
  };
  ...
  const artifactResult = {
    ...result,
    result: { ...result.result, response: JSON.stringify({ ...fullArtifact, toolAction: 'Completing artifact', toolSummary: 'Artifact' }) },
  };
  ...
  const conflicting = {
    ...artifactTool,
    step_update: {
      ...artifactTool.step_update,
      tool_info: { name: 'finish', parameters: { ...partialArtifact, artifact: { ...partialArtifact.artifact, content: 'different' } } },
    },
  };
  assert.throws(() => decodeGemini(lines([init, user, conflicting, artifactResult]), model), /conflicts/);
  assert.throws(
    () => decodeGemini(lines([init, user, { ...finishTool, step_update: { ...finishTool.step_update, tool_name: 'view_file' } }, result]), model),
    /policy violation/,
  );
});
test('Antigravity model failures are distinguishable from policy failures', () => {
  ...
  assert.throws(
    () => decodeGemini(lines([init, user, { ...result, result: { ...result.result, status: 'ERROR', error: 'backend unavailable' } }]), model),
    /model\/provider did not complete/,
  );
});
test('Antigravity nonterminal, duplicate, failed, or unbounded-turn responses reject', () => {
  for (const status of ['ERROR', 'CANCELED', 'INTERRUPTED', 'INVALID', 'WAITING', 'RUNNING'])
    assert.throws(() => decodeGemini(lines([init, user, { ...result, result: { ...result.result, status } }]), model));
  ...
  assert.throws(() =>
    decodeGemini(
      lines([
        init,
        user,
        { ...syntheticInput, step_update: { ...syntheticInput.step_update, duration_seconds: '0.01' } },
        { ...result, result: { ...result.result, num_turns: 2 } },
      ]),
      model,
    ),
  );
...
test('live stdout guard terminates the owned mock worker on prohibited activity', async () => {
  const result = await execute(
    process.execPath,
    ['-e', 'console.log(JSON.stringify({event:"step_update",step_update:{step_type:"tool"}}));setInterval(()=>{},1000)'],
    { env: cleanEnv(), timeoutMs: 3000, onStdoutLine: line => guardGeminiEvent(JSON.parse(line), model) },
  );
...
test('live stdout guard classifies provider error steps without inventing tool activity', async () => {
  const result = await execute(
    process.execPath,
    ['-e', 'console.log(JSON.stringify({event:"step_update",step_update:{step_type:"error_message",state:"DONE"}}));setInterval(()=>{},1000)'],
    { env: cleanEnv(), timeoutMs: 3000, onStdoutLine: line => guardGeminiEvent(JSON.parse(line), model) },
  );
...
test('live log guard terminates the owned mock worker on discovery fallback', async () => {
  const temporary = await tempDir('team-loop-agy-log-');
  ...
  const result = await execute(process.execPath, ['-e', script], {
    ...
    watchFile: logFile,
    checkWatchText: text => {
      if (/Agent "team-loop-reviewer" not found, falling back to default/.test(text)) throw Error('discovery failed');
    },
  });
  assert.equal(result.stopped, 'worker-policy-violation');
