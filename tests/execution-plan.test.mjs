...
async function planFixture() {
  const root = await tempDir('teamloop-plan-');
...
test('execution plan rejects shell-shaped stages, cycles, missing dependencies and unsafe concurrency', async () => {
  ...
  const cycle = {
    ...plan,
    stages: [
      { ...plan.stages[0], dependsOn: ['second-review'] },
      { ...plan.stages[1], dependsOn: ['first-review'] },
    ],
  };
...
test('scheduler respects dependencies, concurrency and one active stage per provider', async () => {
  const root = await tempDir('teamloop-plan-scheduler-');
  ...
  const plan = {
    ...
    stages: [
      ...
      },
      {
        ...
        type: 'review',
        description: '',
        dependsOn: ['alpha-one', 'beta-one', 'alpha-two'],
        failureMode: 'required',
        packetFile: packet,
        provider: 'gamma',
        model: 'one',
...
test('required failure cancels dependents while advisory failure remains visible and permits them', async () => {
  const root = await tempDir('teamloop-plan-failures-');
  const packet = path.join(root, 'packet.json');
  await fs.writeFile(packet, '{}');
  const baseStage = { type: 'review', description: '', packetFile: packet, model: 'one', effort: 'default' };
  const worker = async (_packet, provider) =>
    provider.startsWith('fail')
      ? { status: 'failed', failureClass: 'fixture', error: 'fixture failure' }
      : { runId: crypto.randomUUID(), status: 'completed', packetHash: 'hash', verdict: 'approve' };
  const required = {
    schemaVersion: 1,
    taskId: 'required-failure',
    policyVersion: 'test-1',
    maxConcurrency: 2,
    stages: [
  ...
  assert.deepEqual(
    requiredResult.stages.map(stage => stage.status),
    ['failed', 'canceled'],
  );
  const advisory = {
    schemaVersion: 1,
    taskId: 'advisory-failure',
    policyVersion: 'test-1',
    maxConcurrency: 2,
    stages: [
...
test('unexpected scheduler failure records a terminal plan status and event', async () => {
  const root = await tempDir('teamloop-plan-recovery-');
  const packet = path.join(root, 'packet.json');
  await fs.writeFile(packet, '{}');
  const plan = {
    schemaVersion: 1,
    taskId: 'scheduler-recovery',
    policyVersion: 'test-1',
    maxConcurrency: 1,
    stages: [
      {
  ...
  await assert.rejects(
    runExecutionPlan(plan, { stateRoot: path.join(root, 'state') }, async () => ({ status: 'completed' }), { output: null }),
    /status/,
  );
  ...
  assert.equal(status.failureClass, 'plan-runner');
...
test('mock plan runs end to end through the existing bounded worker runner', async () => {
  const root = await tempDir('teamloop-plan-integration-');
  ...
  const packet = await prepare(
    {
      id: 'plan-integration',
      root,
      assignment: 'Review the supplied fixture',
      requirements: ['R1'],
      constraints: [],
      decisions: [],
      files: ['brief.md'],
  ...
  let output = '';
  const result = await runExecutionPlan(plan, config, run, {
    output: {
      write: value => {
        output += value;
      },
  ...
  assert.equal(result.status, 'completed');
  assert.equal(result.stages[0].verdict, 'blocked');
...
test('plan CLI validates, shows, graphs and emits an NDJSON mock run', async () => {
  const root = await tempDir('teamloop-plan-cli-');
  ...
  const packet = await prepare(
    {
      id: 'plan-cli',
      root,
      assignment: 'Review the CLI fixture',
      requirements: ['R1'],
      constraints: [],
      decisions: [],
      files: ['brief.md'],
  ...
  const cli = (...args) =>
    spawnSync(process.execPath, [path.join(repo, 'src/team-loop.mjs'), 'plan', ...args, configFile], { cwd: repo, encoding: 'utf8', windowsHide: true });
  const validated = cli('validate', planFile);
  assert.equal(validated.status, 0, validated.stderr);
  assert.equal(JSON.parse(validated.stdout).valid, true);
  const shown = cli('show', planFile);
  assert.equal(shown.status, 0, shown.stderr);
  assert.equal(JSON.parse(shown.stdout).stages[0].packetFile, packet);
  const graph = cli('graph', planFile);
  assert.equal(graph.status, 0, graph.stderr);
  assert.match(graph.stdout, /mock-review/);
  const ran = cli('run', planFile);
