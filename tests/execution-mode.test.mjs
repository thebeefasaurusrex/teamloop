...
const config=stateRoot=>({stateRoot,standardPolicyVersion:'1.8.0',executionModes:{'claude-bridge':{enabled:true,provider:'claude',builderVersion:'1.0.0',validatedBuilderVersion:'1.0.0'}}});

test('execution overlay is opt-in, expiring, and leaves standard canonical',async()=>{
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'teamloop-mode-'));
  const cfg=config(root),now=Date.parse('2026-09-13T12:00:00Z');
...
test('invalid, stale, disabled, or unvalidated overlay state fails closed to standard',async()=>{
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'teamloop-mode-'));
  const cfg=config(root),file=path.join(root,'execution-mode.json'),now=Date.parse('2026-09-13T12:00:00Z');
  ...
  assert.throws(()=>validateExecutionModes({...cfg,executionModes:{'claude-bridge':{enabled:true,provider:'claude',builderVersion:'1.0.1',validatedBuilderVersion:'1.0.0'}}}));
});
