...
test('evaluation math keeps failure denominators, raw outcomes and verification distinct',()=>{
  ...
  assert.equal(result.completedWorkMean,80);assert.equal(result.reliabilityAdjustedMean,40);assert.equal(result.strictVerifiedDeliveryMean,40);assert.equal(result.reliabilityPercent,50);assert.equal(result.raw.length,2);
  const changed=structuredClone(fixtures);changed[0].verification='failed';
  assert.equal(summarize(changed)[0].strictVerifiedDeliveryMean,0);assert.equal(summarize(changed)[0].completedWorkMean,80);
});
test('unknown completed scores are not imputed and preflight refusals remain visible',()=>{
  const rows=structuredClone(fixtures);rows[0].score=null;
  assert.equal(summarize(rows)[0].completedWorkMean,null);assert.equal(summarize(rows)[0].reliabilityAdjustedMean,null);
  rows[1].delivery='not-started';assert.equal(summarize(rows)[0].notStarted,1);assert.equal(summarize(rows)[0].attempts,1);
});
