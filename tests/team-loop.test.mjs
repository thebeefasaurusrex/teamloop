...
import {hash, safeText, contained, readSource, readImage, prepare, checkPacket, cleanEnv, execute, validateResponse, validateArtifact, outputSchema, decode, run, recentAttestation, workerFailure, assertImageTransport} from '../src/team-loop.mjs';

const temp=await fs.mkdtemp(path.join(os.tmpdir(),'team-loop-tests-'));
const root=path.join(temp,'project');
await fs.mkdir(root);
await fs.writeFile(path.join(root,'brief.md'),'A small synthetic task.\n');
const config={
  schemaVersion:1,stateRoot:path.join(temp,'state'),blockedLiterals:['excluded@example.test'],
...
const artifact={packetHash:artifactPacket.packetHash,artifact:{filename:'example.html',mediaType:'text/html',content:'<!doctype html><html><body><button>Works</button></body></html>'},rationale:'A functional minimal example.',uncertainties:[]};

test('explicit file packet has stable content hash',async()=>{await checkPacket(packet);assert.equal(await prepare(spec,config),packetFile);});
...
test('artifact byte ceiling is disclosed, packet-bound, and enforced',async()=>{
  ...
  assert.throws(()=>validateArtifact({...artifact,artifact:{...artifact.artifact,content:'<!doctype html><html><body>'+('x'.repeat(20000))+'</body></html>'}},artifactPacket.packetHash,16384),/disclosed/);
  for(const maxArtifactBytes of [16383,1048577,32768.5,'32768']) await assert.rejects(prepare({...spec,id:'bad-artifact-limit',kind:'artifact',maxArtifactBytes},config),/byte limit/);
...
test('identical tasks in distinct roots do not reuse the wrong project',async()=>{
  const second=path.join(temp,'second');await fs.mkdir(second);
  ...
  assert.equal(secondPacket.root,second);await checkPacket(secondPacket);
...
test('legacy packets require explicit repreparation, not a silent compatibility fallback',async()=>{
  const legacy=structuredClone(packet);legacy.payload.version=2;legacy.packetHash=hash(legacy.payload);
...
test('reject stale and tampered packets',async()=>{
  ...
  const altered=structuredClone(packet); altered.payload.sources[0].content='changed'; altered.packetHash=hash(altered.payload);
...
test('reject malformed, empty, mismatched and failed responses',()=>{
  ...
  assert.deepEqual(decode('claude',JSON.stringify({type:'result',subtype:'success',num_turns:1,result:JSON.stringify(review)}),packet.packetHash).review,review);
});
test('artifact packets decode complete self-contained HTML and reject unsafe output',()=>{
  ...
  assert.deepEqual(decode('claude',JSON.stringify({type:'result',subtype:'success',num_turns:1,result:JSON.stringify(artifact)}),artifactPacket.packetHash,null,'artifact',artifactPacket.payload.maxArtifactBytes).review,artifact);
  assert.throws(()=>validateArtifact({...artifact,artifact:{...artifact.artifact,filename:'../bad.html'}},artifactPacket.packetHash));
  assert.throws(()=>validateArtifact({...artifact,artifact:{...artifact.artifact,content:'<!doctype html><html><body>truncated'}},artifactPacket.packetHash),/Invalid artifact/);
  assert.throws(()=>validateArtifact({...artifact,artifact:{...artifact.artifact,content:'<!doctype html><html><body><script src="https://bad.example/x.js"></script></body></html>'}},artifactPacket.packetHash),/self-contained/);
});
...
test('Claude accepts schema-normalization turns and rejects incomplete or tool-active results',()=>{
  ...
  assert.deepEqual(decode('claude',JSON.stringify({...envelope,num_turns:2,stop_reason:'tool_use',terminal_reason:'completed'}),packet.packetHash).review,review);
  for(const change of [{type:'message'},{subtype:'error_max_turns'},{num_turns:0},{terminal_reason:'model_error'},{usage:{server_tool_use:{web_search_requests:1}}},{permission_denials:[{tool_name:'Read'}]}]) assert.throws(()=>decode('claude',JSON.stringify({...envelope,...change}),packet.packetHash));
});
...
test('child environment excludes provider keys, redirects and proxy settings',()=>{
  assert.deepEqual(cleanEnv({PATH:'ok',ANTHROPIC_API_KEY:'secret',OPENAI_API_KEY:'secret',GOOGLE_API_KEY:'secret',ANTHROPIC_BASE_URL:'bad',HTTP_PROXY:'bad',NODE_OPTIONS:'bad',GEMINI_CLI_HOME:'bad'}),{PATH:'ok'});
});
...
test('nonzero exits and stderr remain observable',async()=>{
  ...
  assert.equal(result.code,3);assert.match(result.stderr,/fixture failure/);
...
test('cancellation marker terminates owned child',async()=>{
  const marker=path.join(temp,'cancel.request');await fs.writeFile(marker,'cancel');
...
test('policy and unknown-model gates happen before authentication',async()=>{
  ...
  const apiConfig={...config,allowApiSpend:true,providers:{nvidia:{enabled:true,models:{'example/model':{efforts:['default'],imageInput:false}}}}};
  await assert.rejects(run(packetFile,'nvidia','example/model',apiConfig),/public or synthetic/);
});

console.log('Disposable synthetic fixtures retained at '+temp);
