...
async function fixture() {
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'teamloop-public-'));
  const stateRoot=path.join(root,'state');
  await fs.writeFile(path.join(root,'brief.md'),'Synthetic evidence.');
  const config={schemaVersion:1,stateRoot,blockedLiterals:[],limits:{maxTaskTimeoutMs:180000,maxRunsPerProviderPacket:1},providers:{mock:{enabled:true,models:{fixture:{efforts:['default'],imageInput:false}}}}};
  const spec={id:'public-test',root,assignment:'Review supplied evidence',requirements:['R1'],constraints:[],decisions:[],files:['brief.md'],dataClass:'synthetic'};
...
test('no-network runner completes a real child-process round trip and records unknown usage',async()=>{
  ...
  assert.equal(record.status,'completed');assert.equal(record.verdict,'blocked');assert.equal(record.usage,null);
...
test('attempt cap prevents another worker start and preserves the refusal',async()=>{
  ...
  assert.equal(second.status,'failed');assert.match(second.error,/budget exhausted/);assert.equal(second.exitCode,undefined);
  assert.deepEqual(await fs.readdir(path.join(config.stateRoot,'locks')),[]);
});
...
test('invalid packet timeout is rejected before leaving a lock behind',async()=>{
  ...
  invalid.payload.timeoutMs=900001;invalid.packetHash=hash(invalid.payload);
...
test('provider opt-in, accounts, API spend and reasoning effort fail closed',async()=>{
  ...
  config.allowApiSpend=true;validateConfig(config);
...
test('relative config paths resolve from their file, not the invoking directory',async()=>{
  ...
  assert.equal(loaded.stateRoot,path.join(root,'separate-state'));assert.equal(loaded.codexHome,path.join(root,'auth'));
});
test('routing is roster-driven and explains unavailable roles without executing them',async()=>{
  const {config}=await fixture();
  const profile={needs:['engineering','critique'],stakes:'medium',requestedTeam:true,maxReviewers:2,availableProviders:['mock'],leadFamily:'lead'};
  const candidate={provider:'mock',model:'fixture',effort:'default',family:'fixture'};
  const roster={schemaVersion:1,roles:{engineering:[candidate],critique:[candidate]},toolChecks:{engineering:['Run tests'],critique:['Check requirements']}};
  const first=route(profile,roster,config);
  assert.equal(first.workers.length,1);assert.equal(first.unfilled.length,1);assert.deepEqual(first.toolChecks,['Run tests','Check requirements']);
...
test('likely provider secrets are blocked and redacted without containing real keys',()=>{
  for(const secret of [['nvapi','x'.repeat(30)].join('-'),['sk-ant','x'.repeat(30)].join('-'),['ghp','x'.repeat(30)].join('_')]) {
    assert.throws(()=>safeText(secret));assert.equal(redact(secret),'[REDACTED]');
  }
});
test('clean install is self-contained, works from another cwd, and refuses overwrite',async()=>{
  ...
  const install=()=>spawnSync(process.execPath,[path.join(repo,'scripts/install-skill.mjs'),'--dest',destination],{cwd:root,encoding:'utf8',windowsHide:true});
  const installed=install();assert.equal(installed.status,0,installed.stderr);
  ...
  const taskFile=path.join(root,'task.json');const configFile=path.join(root,'config.local.json');
  await fs.writeFile(taskFile,JSON.stringify(spec));await fs.writeFile(configFile,JSON.stringify(config));
  ...
  const ran=spawnSync(process.execPath,[wrapper,'run',prepared.stdout.trim(),'mock','fixture',configFile],{cwd:os.tmpdir(),encoding:'utf8',windowsHide:true});
  assert.equal(ran.status,0,ran.stderr);assert.equal(JSON.parse(ran.stdout).status,'completed');
});
test('changed images are rejected before transport reads and exact bytes are retained',async()=>{
  ...
  const loaded=await loadImages(packet);assert.equal(loaded[0].data,bytes.toString('base64'));
...
test('incomplete foreign run records do not stop unrelated dispatch',async()=>{
  ...
  const result=await run(packet,'mock','fixture',config);assert.equal(result.status,'completed');
});
test('runtime output validation rejects extra schema properties',()=>{
  ...
  assert.throws(()=>validateResponse({...review,findings:[{severity:'low',requirement:'r',evidence:'e',recommendation:'r',unexpected:true}]},'abc'),/properties/);
  assert.throws(()=>validateArtifact({packetHash:'abc',artifact:{filename:'x.html',mediaType:'text/html',content:'<!doctype html><html></html>',unexpected:true},rationale:'Fixture',uncertainties:[]},'abc'),/properties/);
});
test('status lists incomplete and corrupt records without hiding valid runs or editing evidence',async()=>{
  ...
  const result=await run(packet,'mock','fixture',config);assert.equal(result.status,'completed');
  await fs.mkdir(incomplete);await fs.mkdir(corrupt);await fs.writeFile(path.join(corrupt,'status.json'),'{');
  ...
  const refusal=await run(packet,'mock','fixture',config);assert.equal(refusal.status,'failed');assert.match(refusal.error,/reconcile/);
