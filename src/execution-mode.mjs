...
export function validateExecutionModes(config) {
  ...
  for(const [name,mode] of Object.entries(modes)) {
    if(name===STANDARD_MODE||!validName(name)||!mode||typeof mode!=='object'||Array.isArray(mode)) throw Error('Invalid execution mode definition');
    if(typeof mode.enabled!=='boolean'||typeof mode.provider!=='string'||!validVersion(mode.builderVersion)||!validVersion(mode.validatedBuilderVersion)) throw Error('Execution mode requires enabled, provider, builderVersion, and validatedBuilderVersion');
...
export async function resolveExecutionMode(config,now=Date.now()) {
  const base={requestedMode:STANDARD_MODE,effectiveMode:STANDARD_MODE,standardPolicyVersion:config.standardPolicyVersion??null,overlay:null,reason:'no-selection'};
  let state;
  try {state=JSON.parse(await fs.readFile(stateFile(config),'utf8'));}
  catch(error) {
    if(error.code==='ENOENT') return base;
    if(error instanceof SyntaxError) return {...base,requestedMode:null,reason:'invalid-state'};
    throw error;
  }
  if(!state||typeof state!=='object'||Array.isArray(state)||!validName(state.requestedMode)) return {...base,requestedMode:null,reason:'invalid-state'};
  ...
  if(!validVersion(mode.builderVersion)||mode.builderVersion!==mode.validatedBuilderVersion||state.validatedBuilderVersion!==mode.validatedBuilderVersion) return {...base,requestedMode:state.requestedMode,reason:'unvalidated-builder'};
  ...
  return {requestedMode:state.requestedMode,effectiveMode:state.requestedMode,standardPolicyVersion:config.standardPolicyVersion,overlay:{provider:mode.provider,builderVersion:mode.builderVersion,activatedAt:state.activatedAt,expiresAt:state.expiresAt,activatedBy:state.activatedBy},reason:'active'};
}
...
export async function activateExecutionMode(config,name,expiresAt,{now=Date.now(),activatedBy='explicit-user-decision'}={}) {
  ...
  const state={schemaVersion:1,requestedMode:name,activatedAt:new Date(now).toISOString(),expiresAt:new Date(expires).toISOString(),activatedBy,validatedBuilderVersion:modes[name].validatedBuilderVersion,standardPolicyVersion:config.standardPolicyVersion};
...
export async function selectStandardMode(config,{now=Date.now(),activatedBy='explicit-user-decision'}={}) {
  ...
  const state={schemaVersion:1,requestedMode:STANDARD_MODE,activatedAt:new Date(now).toISOString(),activatedBy,standardPolicyVersion:config.standardPolicyVersion??null};
  await atomicJson(stateFile(config),state);
  return resolveExecutionMode(config,now);
}
