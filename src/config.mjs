...
export function validateConfig(config) {
  if (config?.schemaVersion !== 1 || typeof config.stateRoot !== 'string' || !path.isAbsolute(config.stateRoot))
    throw Error('Configuration requires schemaVersion 1 and a stateRoot path');
  ...
  if (!Array.isArray(config.blockedLiterals) || config.blockedLiterals.some(v => typeof v !== 'string' || !v.trim()))
    throw Error('blockedLiterals must be an array of nonempty strings');
  const limits = config.limits;
  if (
    !limits ||
    !Number.isSafeInteger(limits.maxTaskTimeoutMs) ||
    limits.maxTaskTimeoutMs < 30000 ||
    limits.maxTaskTimeoutMs > 900000 ||
    !Number.isSafeInteger(limits.maxRunsPerProviderPacket) ||
    limits.maxRunsPerProviderPacket < 1 ||
    limits.maxRunsPerProviderPacket > 10
  )
    throw Error('Set limits: maxTaskTimeoutMs (30000-900000), maxRunsPerProviderPacket (1-10)');
  for (const [provider, settings] of Object.entries(config.providers)) {
    if (!providerIds.has(provider) || !settings || typeof settings.enabled !== 'boolean')
      throw Error('Unsupported provider or missing enabled flag: ' + provider);
    ...
    for (const [model, profile] of Object.entries(settings.models)) {
      if (
        ...
        !profile ||
        !Array.isArray(profile.efforts) ||
        !profile.efforts.length ||
        profile.efforts.some(e => !/^[a-z][a-z0-9-]{0,23}$/.test(e)) ||
        typeof profile.imageInput !== 'boolean'
      )
        throw Error('Invalid model profile');
      if (
        profile.resolvedModels !== undefined &&
        (!Array.isArray(profile.resolvedModels) || !profile.resolvedModels.length || profile.resolvedModels.some(m => typeof m !== 'string' || !m))
      )
        throw Error('resolvedModels must be a nonempty list');
    }
    if (!settings.enabled) continue;
    if (!Object.keys(settings.models).length) throw Error('Enabled provider needs an explicit model allowlist');
    if (
      accountProviders.includes(provider) &&
      (!Array.isArray(settings.allowedAccounts) ||
        !settings.allowedAccounts.length ||
        settings.allowedAccounts.some(email => typeof email !== 'string' || !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)))
    )
      throw Error('Enabled subscription provider requires allowedAccounts');
