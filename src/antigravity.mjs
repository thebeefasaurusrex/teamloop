...
export async function ensureGeminiAgent(config, env = process.env, install = false) {
  ...
  for (const [name, content] of [
    ['team-loop-reviewer', reviewerAgent],
    ['team-loop-designer', designerAgent],
  ]) {
...
export function checkGeminiSettings(settings) {
  ...
  if (
    ...
    (settings.allowNonWorkspaceAccess !== undefined && settings.allowNonWorkspaceAccess !== false) ||
    !Array.isArray(settings.permissions?.deny) ||
    !denyRules.every(rule => settings.permissions.deny.includes(rule))
  )
    throw Error('Antigravity personal-subscription, privacy, or tool-denial settings are unsafe');
}
...
export async function geminiIdentity(config, env, execute) {
  if (config.providers?.gemini?.enabled !== true || !config.antigravity || !config.antigravityHome || config.antigravityFileLoginCompatibility !== true)
    throw Error('Antigravity adapter or file-login compatibility is not configured');
  ...
  if (
    agents.code !== 0 ||
    agents.stopped ||
    !['team-loop-reviewer', 'team-loop-designer'].every(name => new RegExp(`(?:^|\\s)${name}(?:\\s|$)`).test(agents.stdout))
  )
    throw Error('Antigravity did not discover the required team-loop agents');
  await enforceGeminiSettings(config);
  ...
  try {
    ...
    throw Error('Antigravity personal identity verification failed');
  }
  if (!response.ok) throw Error('Antigravity personal identity verification failed: HTTP ' + response.status);
  const identity = await response.json();
  if (!identity.verified_email || !config.providers.gemini.allowedAccounts.includes(identity.email))
    throw Error('Antigravity must use an explicitly allowed verified Google account');
...
export function guardGeminiEvent(event, model, agent = 'team-loop-reviewer') {
  ...
  if (event.event === 'init' && (event.init?.permission_mode !== 'strict' || event.init?.model !== model || event.init?.agent !== agent))
    throw Error('Unexpected Antigravity runtime configuration');
  if (event.event === 'step_update') {
    ...
    const structuredFinish =
      ...
      step.tool_info?.name === 'finish' &&
      step.tool_info.parameters &&
      typeof step.tool_info.parameters === 'object' &&
      !step.subagent_info;
    if (
      !structuredFinish &&
      (step.tool_name || step.tool_info || step.subagent_info || !['user_input', 'agent_response', 'checkpoint', 'finish'].includes(step.step_type))
    )
      throw Error('Antigravity policy violation: forbidden tool or subagent activity');
  ...
  if (
    ...
    (event.result?.status !== 'SUCCESS' ||
      event.result.error ||
      !Number.isSafeInteger(event.result.num_turns) ||
      event.result.num_turns < 1 ||
      event.result.num_turns > 3)
  )
    throw Error(
      'Antigravity model/provider did not complete one bounded structured turn: ' + (event.result?.error || event.result?.status || 'invalid turn count'),
    );
}
...
export function decodeGemini(output, model, agent = 'team-loop-reviewer') {
  ...
  if (
    ...
    events.filter(e => e.event === 'init').length !== 1 ||
    events.filter(e => e.event === 'result').length !== 1
  )
    throw Error('Incomplete or repeated Antigravity stream');
  ...
  if (inputs.length !== turns || inputs.some(e => e.step_update.duration_seconds !== undefined && Number(e.step_update.duration_seconds) !== 0))
    throw Error('Antigravity input steps must match its bounded internal structured turns');
  ...
  if (finishTools.length) {
    ...
    try {
      ...
      for (const event of finishTools) {
        ...
        if (partial.packetHash !== undefined && response.packetHash !== undefined && partial.packetHash !== response.packetHash)
          throw Error('Antigravity structured finish tool conflicts with terminal result');
        if (partial.artifact && response.artifact) {
          for (const key of ['filename', 'mediaType'])
            if (partial.artifact[key] !== undefined && response.artifact[key] !== undefined && partial.artifact[key] !== response.artifact[key])
              throw Error('Antigravity structured finish tool conflicts with terminal result');
...
export async function geminiInvocation(
  config,
  runDir,
  model,
  prompt,
  env,
  agent = 'team-loop-reviewer',
  timeoutMs = 180000,
  schema = null,
  effort = 'default',
) {
  ...
  return {
    ...
    env: geminiEnv(env, config),
    input: JSON.stringify({ event: 'user', message: { content: prompt } }) + '\n',
    onStdoutLine: line => guardGeminiEvent(JSON.parse(line), model, agent),
    watchFile: logFile,
    checkWatchText: text => {
      if (new RegExp(`Agent "${agent}" not found, falling back to default`, 'i').test(text)) throw Error('Antigravity agent discovery failed closed');
      if (/inherited useG1Credits=true/i.test(text)) throw Error('Antigravity paid-credit fallback is enabled');
    },
  };
}
