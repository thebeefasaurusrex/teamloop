...
import { atomicJson, hash, readJson } from './shared.mjs';
...
const assertKeys = (value, allowed, label) => {
  if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some(key => !allowed.includes(key)))
    throw Error(`Invalid ${label} properties`);
};

function validateGraph(stages) {
  const byId = new Map(stages.map(stage => [stage.id, stage]));
  for (const stage of stages)
    for (const dependency of stage.dependsOn) if (!byId.has(dependency)) throw Error(`Unknown dependency ${dependency} for stage ${stage.id}`);
...
export async function resolveExecutionPlan(plan, planFile, config, { modelResolver = resolveModel } = {}) {
  ...
  for (const raw of plan.stages) {
    ...
    if (raw.description !== undefined && (typeof raw.description !== 'string' || !raw.description.trim()))
      throw Error(`Invalid description for stage ${raw.id}`);
    const dependsOn = raw.dependsOn ?? [];
    if (!Array.isArray(dependsOn) || new Set(dependsOn).size !== dependsOn.length || dependsOn.some(id => !idPattern.test(id)))
      throw Error(`Invalid dependencies for stage ${raw.id}`);
...
function publicStageState(state) {
  ...
  for (const key of ['startedAt', 'finishedAt', 'workerRunId', 'packetHash', 'verdict', 'failureClass', 'error', 'reason'])
    if (state[key] != null) value[key] = state[key];
  return value;
}
...
export async function runExecutionPlan(
  ...
  config,
  runWorker,
  { output = process.stdout, sanitizeError = value => String(value), planSource = 'inline' } = {},
) {
  ...
  const states = new Map(
    plan.stages.map(stage => [stage.id, { id: stage.id, status: 'waiting', failureMode: stage.failureMode, provider: stage.provider, model: stage.model }]),
  );
  const status = {
    schemaVersion: 1,
    planRunId,
    ...
    startedAt,
    status: 'running',
    leadDecisionRequired: true,
    stages: [...states.values()].map(publicStageState),
  };
  ...
  const persist = async () => {
    status.stages = plan.stages.map(stage => publicStageState(states.get(stage.id)));
    await atomicJson(statusFile, status);
  };
  ...
  const launch = async stage => {
    ...
    activeProviders.add(stage.provider);
    await emit({
      event: 'stage_started',
      stageId: stage.id,
  ...
  try {
    ...
    while (true) {
      ...
      activeProviders.delete(result.result.stage.provider);
      const state = states.get(result.id),
        record = result.result.record;
      state.status = result.result.status;
      state.finishedAt = new Date().toISOString();
    ...
    await emit({
      ...
      leadDecisionRequired: true,
      stages: status.stages.map(stage => ({ id: stage.id, status: stage.status, workerRunId: stage.workerRunId ?? null })),
    });
    return status;
  } catch (error) {
    ...
    for (const stage of plan.stages) {
      ...
      if (state.status === 'waiting') {
        state.status = 'canceled';
        state.reason = 'plan-runner-failed';
        state.finishedAt = new Date().toISOString();
      }
    }
    status.status = 'failed';
    status.failureClass = 'plan-runner';
  ...
  for (const id of entries) {
    try {
      ...
      if (error.code === 'ENOENT') records.push({ planRunId: id, status: 'incomplete-record', note: 'No plan status was published. No evidence was deleted.' });
        ...
        records.push({ planRunId: id, status: 'corrupt-record', note: 'Reconcile retained plan evidence before another dispatch. No evidence was changed.' });
      else throw error;
    }
  }
  return records;
