...
if(!Array.isArray(files) || new Set(files).size!==files.length || files.some(f=>typeof f!=='string' || !f || path.isAbsolute(f) || f.split('/').some(part=>['..','.',''].includes(part)) || f.includes('\\'))) throw Error('Invalid explicit release allowlist');
...
for(const name of files) {
  ...
  } catch {errors.push('Missing or unsafe release file: '+name);continue;}
  const extension=path.extname(name).toLowerCase();
  const binaryHeaders={'.png':Buffer.from([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a]),'.ttf':Buffer.from([0,1,0,0]),'.woff2':Buffer.from('wOF2')};
  let text='';
  if(Object.hasOwn(binaryHeaders,extension)) {
    ...
    catch {errors.push('Non-UTF8 file without an approved binary type: '+name);}
  }
...
else console.log(JSON.stringify({status:'passed',files:receipts.length,totalBytes:receipts.reduce((sum,f)=>sum+f.bytes,0),limits:'Allowlist and text-pattern checks are not proof of absence of every secret. PNG/TTF/WOFF2 files receive signature and hash checks, not text screening or complete format validation; inspect their source and rendered content separately.',manifest:receipts},null,2));
